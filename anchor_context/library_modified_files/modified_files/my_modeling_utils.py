...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def _check_and_enable_flex_attn(
        ...
        if not is_torch_available():
            if hard_check_only:
                raise ImportError(
                    "Cannot enable flex_attention because PyTorch is not available."
                )
            else:
        ...
        # Check if PyTorch version is >= 2.5.0
        torch_version = torch.__version__
        if torch_version < "2.5.0":
            if hard_check_only:
                raise ImportError(
                    "Cannot enable flex_attention because PyTorch version is less than 2.5.0."
                )
            else:
                config._attn_implementation = "eager"
                return config
    ...
    @classmethod
    def _supports_flex_attn(cls):
        from transformers.utils import is_torch_available
        if not is_torch_available():
            return False
        import torch
        return torch.__version__ >= "2.5.0"
    ...
    def _autoset_attn_implementation(
        ...
        Automatically checks and dispatches to a default attention implementation. In order of priority:
            1. An implementation specified in `config._attn_implementation` (due for example to the argument attn_implementation="flex_attention" in from_pretrained).
            2. DEPRECATED: if use_flash_attention_2 is set to `True` and `flash_attn` is available, flash attention. (`LlamaFlashAttention` for example)
            3. Flex Attention implementation, if available and supported by the model type. (`LlamaFlexAttention` for example)
            4. SDPA implementation, if available and supported by the model type. (`LlamaSdpaAttention` for example)
            5. The default model's implementation otherwise (`LlamaAttention` for example).
        """
        ...
        if hasattr(config, "_attn_implementation_internal") and config._attn_implementation_internal is not None:
            ...
            if config._attn_implementation not in ["eager", "sdpa", "flash_attention_2", "flex_attention"]:
                ...
                if cls._supports_flex_attn():
                    message += ', `"attn_implementation=flex_attention"` (implementation using torch.nn.functional.flex_attention)'
        ...
        if config._attn_implementation == "flash_attention_2":
            ...
            cls._check_and_enable_flex_attn(
                config,
                torch_dtype=torch_dtype,
                device_map=device_map,
                hard_check_only=False,
